...
pygame.display.set_caption('Trò choi 8 số')
...
pygame_gui.core.IWindowInterface.set_display_title(self=window_surface, new_title="tro choi 8 so")
...
display_elements()
### solve button
solve_button = pygame_gui.elements.UIButton(relative_rect=pygame.Rect((1000, 600), (250, 45)),
                                            text='Solve Puzzle',
                                            manager=manager,
                                            object_id="#solve_btn")

### algorithmOptions DropDown
dropdown_layout_rect = pygame.Rect((970, 550), (280, 35))
...
def solveAnimation(moves):
    for mv in moves:
        ...
        time.sleep(0.2)
#di chuyển từng bước trong quá trình giải quyết trò chơi 8 số và
# hiển thị trực quan các bước di chuyển trên giao diện đồ họa.
        ...
        if event.type == pygame.USEREVENT:
            if event.user_type == pygame_gui.UI_BUTTON_PRESSED:
                if event.ui_element == shuffle_button:
                    ...
                    Info_msg = '<b>8-Puzzle Solver<br><br>Authors:===ư</b><br>Mateus Mendonça Monteiro<br>Vinicius Santana Ramos'
                    # Information Box - Info
                    info_win = pygame_gui.windows.ui_confirmation_dialog.UIConfirmationDialog(
                        rect=pygame.Rect((600, 300), (180, 80)),
                        manager=manager,
                    ...
                    if algorithm == "Best-First (Manhatan Distance)":
                        ...
                        report_msg = '<b>cac nut đa truy cap:</b> ' + str(
                            puzzle.cost) + '          <b>Thoi gian:</b>' + tempo + '        <b>so buoc da dung:</b> ' + str(
                            len(moves)) + ' steps'
                        # Confirmation Box - Algorithm Report
                        confirmation_win = pygame_gui.windows.ui_confirmation_dialog.UIConfirmationDialog(
                            rect=pygame.Rect((600, 300), (180, 80)),
                            manager=manager,
